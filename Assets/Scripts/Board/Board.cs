...
public class Board
{
    ...
    {
        Vector3 origin = new Vector3(-boardSizeX * 0.5f + 0.5f, -boardSizeY * 0.5f + 1.5f, 0f);
    ...
    internal void Fill()
    {
        List<NormalItem> boardItems = new List<NormalItem> ();
        List<NormalItem.eNormalType> allTypes = Utils.GetAllNormalTypes();
        while(boardItems.Count < boardSizeX * boardSizeY) 
        {
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                NormalItem item = boardItems[index++];
